        ...
        <div class="row wrapper border-bottom white-bg page-heading">
            ...
            <div class="col-sm-8">
                <div class="title-action">
                    <a href="{{ route('admin.admin-user.create') }}" class="btn btn-primary">+ Create New</a>
                </div>
            </div>
        </div>
                                ...
                                <table class="table table-striped table-bordered table-hover dataTables-example">
                                    ...
                                    <tbody>
                                        @foreach ($admins as $admin)
                                            <tr class="gradeX" id="row-{{ $admin->id }}">
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $admin->name }}</td>
                                                <td>{{ $admin->email }}</td>
                                                <td>{{ $admin->phone }}</td>
                                                <td>{{ $admin->role }}</td>
                                                <td class="text-right">{{ date('d-M-Y', strtotime($admin->created_at)) }}
                                                </td>
                                                ...
                                                <td class="text-center">
                                                    <div class="btn-group">
                                                        ...
                                                        <a href="{{ route('admin.admin-user.edit', $admin) }}"
                                                            class="btn-white btn btn-xs">Edit</a>
                                                        <button onclick="deleteRecord({{ $admin->id }})"
        ...
        function deleteRecord(id) {
            swal({
                ...
                $.ajax({
                    ...
                    url: "{{ route('admin.admin-user.destroy', '') }}/" + id,
